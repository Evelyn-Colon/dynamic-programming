...
The above pseudocode constructs the length of HVLCS from the solution array `M`. It's very similar to the backtracking algorithm in which we construct the actual LCS--in the source code--but we instead add 1 to an integer value when we would normally be adding a character to a string. Otherwise, it is basically the same algorithm.
